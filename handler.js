...
const newListData = async (type) => {
  ...
  if (type.storeType == "new") {
    ...
    if (convertedFileDownloadSize.type == 'MB') {
      ...
      if (convertedFileDownloadSize.bytes <= 30 && convertedFileDownloadSize >= 20) {
        chunks = 5;
      }
    }
    ...
    if (dataExists) {
      ...
      if (dataDeleted) {
        ...
        if (convertedFileDownloadSize.type == 'MB') {
          ...
          if (convertedFileDownloadSize.bytes <= 30 && convertedFileDownloadSize.bytes >= 20) {
            chunks = 5;
          }
        }
        ...
        result = { listdataDeleted: dataDeleted, listerror: "Problem deleting old data" }
        return result;
      }
  ...
  if(response)
  {
      ...
      if (newUpdate === true) {
        ...
        if (metadataDeleted) {
          ...
          const {metadataAdded, metadata, error} = await metaCreate(response.data)
